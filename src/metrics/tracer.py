...
def compute_tracer_metrics(
    ...
    Args:
        pred_s: Predicted tracer field (..., Nx, Ny).
        target_s: Target tracer field (..., Nx, Ny).
        initial_s: Optional initial condition tracer field (..., Nx, Ny).
            If provided, maximum principle bounds are evaluated against initial_s.
            If None, bounds fallback to target_s.
    ...
    ref_s = initial_s if initial_s is not None else target_s
    s_min = torch.amin(ref_s, dim=(-2, -1), keepdim=True)
    s_max = torch.amax(ref_s, dim=(-2, -1), keepdim=True)

    out_of_bounds = (pred_s < (s_min - 1e-2)) | (pred_s > (s_max + 1e-2))
    out_of_bounds_rate = out_of_bounds.float().mean()

    # 3. Robust L1-normalized integral mass conservation error
    # Avoids near-zero denominator amplification when target tracer has zero spatial mean:
    # |integral(s_pred) - integral(s_target)| / (integral(|s_target|) + eps)
    mass_pred = torch.sum(pred_s, dim=(-2, -1))
    mass_target = torch.sum(target_s, dim=(-2, -1))
    l1_mass_target = torch.sum(torch.abs(target_s), dim=(-2, -1))
    mass_err = torch.mean(torch.abs(mass_pred - mass_target) / (l1_mass_target + eps))

    # 4. Mean conservation error (direct spatial mean drift)
