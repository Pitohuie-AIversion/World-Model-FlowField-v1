"""Rollout evaluation pipeline across future horizons."""
...
def evaluate_rollout_trajectory(
    ...
    """Evaluate multi-step predictions in physical space."""
    ...
    for step in evaluation_steps:
        ...
        p = pred_trajectory[:, idx]
        t = target_trajectory[:, idx]
        step_res = {}

        step_res.update(evaluate_field_metrics(p, t))
        step_res.update(compute_tracer_metrics(p[:, 3], t[:, 3]))
        ...
        spec_res = compute_spectral_error(
            p[:, 0], p[:, 1], t[:, 0], t[:, 1], domain_size=domain_size
        )
