...
def analyze_failure_cases(
    ...
    # Safety check if explicit model_path was provided
    lambda_div = float(cfg.get("lambda_div", 0.0) or 0.0)
    lambda_vort = float(cfg.get("lambda_vort", 0.0) or 0.0)
    if (lambda_div != 0.0 or lambda_vort != 0.0) and cfg.get("physics_protocol") != PHYSICS_PROTOCOL:
        raise RuntimeError(
            f"Checkpoint '{model_path}' has non-zero physics losses (lambda_div={lambda_div}, lambda_vort={lambda_vort}) "
            f"under protocol={cfg.get('physics_protocol')!r}. Only {PHYSICS_PROTOCOL} is allowed."
        )
