...
│   ├── x                  (256,)   [float32] (范围 [0.0, 1.0]，物理长 $L_x = 1.0$)
│   └── y                  (512,)   [float32] (范围 [0.0, 1.0]，物理长 $L_y = 2.0$)
├── scalars/
│   ├── Reynolds           ()       [float32]
│   └── Schmidt            ()       [float32]
...
| 属性 | 实际数值 | 说明 |
| :--- | :--- | :--- |
| **原始空间分辨率** | $256 \times 512$ | 均匀笛卡尔网格（$N_x \times N_y$） |
| **适配训练分辨率** | $128 \times 256$（2x 下采样基线训练）/ $256 \times 512$（高精全分辨率） | 保持周期性边界 2x 规则网格下采样或全分辨率 |
